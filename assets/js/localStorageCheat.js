...
class Book {
    constructor(title = 'Unknown', author = 'Unknown', totalPages = 0, completedPages = 0, completed = false) {
        this.id = ++uniqueId;
        this.title = title;
        this.author = author;
        this.totalPages = totalPages;
...
function BookContainer(book) {
    ...
    this.buttonPagesSubtract.addEventListener('click', bookDecCompletedPagesHandler);
...
function ModalBookFactory(modalType) {
    if (modalType === 'add' || modalType === 'edit') {
        ...
        this.bookTotalPagesInput.setAttribute('id', `book-${modalType}-total-pages`);
        ...
        this.bookTotalPagesLabel.setAttribute('for', `book-${modalType}-total-pages`);
        ...
        this.bookCompletedPagesInput.setAttribute('id', `book-${modalType}-completed-pages`);
        this.bookCompletedPagesInput.setAttribute('required', '');
        this.bookCompletedPagesInput.validated = false;
        // add input
        this.bookCompletedPagesCheckElement.appendChild(this.bookCompletedPagesInput);
        // create label
        this.bookCompletedPagesLabel = document.createElement('label');
        this.bookCompletedPagesLabel.classList.add('title');
        this.bookCompletedPagesLabel.setAttribute('for', `book-${modalType}-completed-pages`);
        this.bookCompletedPagesLabel.textContent = 'Completed pages';
        // add label
        this.bookCompletedPagesCheckElement.appendChild(this.bookCompletedPagesLabel);
        // create check input
        ...
        this.bookCompletedPagesElement.appendChild(this.bookCompletedPagesCheckElement);
        // add book author element
        ...
        if (modalType == 'add') {
            this.buttonAdd.addEventListener('click', () => {
                if (this.validate()) {
                    ...
                    libraryContainer.insertBefore(newBookWrapper.element, libraryContainer.lastElementChild);
        ...
        this.bookTitleInput.addEventListener('input', (e) => {
            if (!!this.bookTitleInput.value && this.bookTitleInput.value.length <= 50) {
                this.changeToValid(this.bookTitleInput, this.bookTitleLabel, 'Title');
            } else {
                this.changeToNotValid(this.bookTitleInput, this.bookTitleLabel, 'Title cannot be empty and length must be less than 50');
            }
        });
        // validation on author input
        this.bookAuthorInput.addEventListener('input', (e) => {
            if (!!this.bookAuthorInput.value && this.bookAuthorInput.value.length <= 30) {
                this.changeToValid(this.bookAuthorInput, this.bookAuthorLabel, 'Author');
            } else {
                this.changeToNotValid(this.bookAuthorInput, this.bookAuthorLabel, 'Author cannot be empty and length must be less than 30');
            }
        });
        ...
        const validCompletedPagesHandler = (e) => {
            if (
                ...
                this.changeToValid(this.bookCompletedPagesInput, this.bookCompletedPagesLabel, 'Completed pages');
                if (+this.bookTotalPagesInput.value === +this.bookCompletedPagesInput.value) {
        ...
        this.bookTotalPagesInput.addEventListener('input', (e) => {
            if (
                ...
                this.changeToValid(this.bookTotalPagesInput, this.bookTotalPagesLabel, 'Total pages');
                validCompletedPagesHandler();
            } else {
                this.changeToNotValid(
        ...
        this.bookCompletedPagesInput.addEventListener('input', validCompletedPagesHandler);
...
ModalBookFactory.prototype.clearAllInputs = function () {
    if (this.wrapper) {
        ...
        this.clearInput(this.bookTotalPagesInput, this.bookTotalPagesLabel, 'Total pages');
        this.clearInput(this.bookCompletedPagesInput, this.bookCompletedPagesLabel, 'Completed pages');
        this.bookCompleted.checked = false;
    }
};
...
function ModalQueryFactory(modalType) {
    if (modalType === 'storage-selection' || modalType === 'confirmation-selection-storage' || modalType === 'confirmation-delete-library') {
        // create wrapper element
        this.wrapper = document.createElement('div');
        this.wrapper.classList.add(`modal-${modalType}-wrapper`);
        this.wrapper.classList.add('display-none');
        ...
        } else if (modalType === 'confirmation-selection-storage' || modalType === 'confirmation-delete-library') {
...
ModalQueryFactory.prototype.insertInBody = ModalBookFactory.prototype.insertInBody;
...
const initAuthState = function () {
    ...
    function disableUserPanelButtons() {
        ...
        libraryPanel.querySelector('.data .delete-btn-cloud').style.display = 'none';
...
let confirmationSelectionStorageModal = new ModalQueryFactory('confirmation-selection-storage');
let confirmationDeleteLibraryModal = new ModalQueryFactory('confirmation-delete-library');
...
libraryPanel.querySelector('.data .switch-btn').addEventListener('click', (e) => {
    if (isLocal) {
        isLocal = false;
        clearLibrary(myLibrary);
...
libraryPanel.querySelector('.data .delete-btn-local').addEventListener('click', (e) => {
...
libraryPanel.querySelector('.data .delete-btn-cloud').addEventListener('click', (e) => {
    confirmationDeleteLibraryModal.on();
    confirmationDeleteLibraryModal.yesSelectionButton.onclick = () => {
        if (!isLocal) {
            clearLibrary(myLibrary);
...
libraryPanel.querySelector('.element.rewrite .rewrite-btn-local').addEventListener('click', (e) => {
    loadDataFromCloudStorage(true);
});
libraryPanel.querySelector('.element.rewrite .rewrite-btn-cloud').addEventListener('click', (e) => {
    clearLibrary(myLibrary);
    myLibrary = getLibraryFromLocalStorage();
    uniqueId = getUniqueIdFromLocalStorage();
    rewriteInCloud();
...
function toggleBookBgColor(book) {
    if (book.completed) {
        ...
        book.container.buttonPagesComplete.style = 'background-color: rgb(36, 14, 49);';
    } else {
...
class _localStorage {
    ...
    static write(book_collection) {
        let book_array = Array.isArray(book_collection) ? book_collection : [book_collection];
...
class Book {
    ...
    get htmlMarkup() {
        ...
        <div class="single_book scale-in-center ${this.read_status ? 'read' : ''}" id="book-${this.id}">
          ...
          <label class="toggle-control">
            <input type="checkbox" id="read_toggle" ${this.read_status ? 'checked' : 'unchecked'}>
